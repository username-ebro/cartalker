...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.vehicleId || !body.source || !body.reportData) {
    ...
    const parsedData = parseGoodCarReport(body.reportData);
    ...
    const importedReport = await prisma.importedReport.create({
      data: {
        ...
        rawData: typeof body.reportData === 'string' ? body.reportData : JSON.stringify(body.reportData),
        summary
      }
    });
